...
#include "ChineseSimplifiedUnicode.h"

namespace ChineseSimplifiedUnicode {

   const char* specialChars = (const char*)u8R"()";

    const ImWchar* getImguiCharRange() {
       static ImVector<ImWchar> ranges; // 因为 Imgui 内部会持有该指针，所以需要为static，保证不被释放
        if (!ranges.empty())
        {
            return ranges.Data;
        }
        ...
        builder.AddText(specialChars);

        builder.AddRanges(base_ranges); // Add base ranges
